	...
	<?php
		while($a=mysqli_fetch_array($donnee)){
			if(isset($_SESSION['admin'])){
				echo '<td><a href="modif_enseign.php?modif_ensein='.$a['id'].'" ><img style="width: 20px;" src="/gestion/image/editer.png" alt="modifier"></a></td><td><a href="modif_enseign.php?supp_ensein='.$a['id'].'" onclick="return(confirm(\'Etes-vous s�r de vouloir supprimer cette entrée?\ntous les enregistrements en relation avec cette entrée seront perdus\'));"><img style="width: 20px;" src="/gestion/image/supprimer.png" alt="supprimer"></td>';} echo '<td>'.$a['nomcl'].'</td><td>'.$a['promotion'].'</td><td>'.$a['nommat'].'</td><td>'.$a['nom'].'</td><td>S'.$a['numsem'].'</td></tr>';
			}
	?>
	<tbody>
	<tr></tr>
